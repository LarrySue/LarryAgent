...
- [ ] AI 角色场景化：支持多套 system prompt 模板（健康管理、工作协助、代码编写、财务分析、阅读探讨等），通过 config.yaml 的 roles 配置定义
  - [ ] 切换策略：默认自动判断（轻量 LLM 调用），允许手动锁定角色。手动锁定期间检测到明显跨域意图时主动提示切换。P1 阶段先支持手动切换（/api/chat 接收 role 字段），P2 再加自动判断
- [ ] 意图识别机制：对话开头快速分类用户意图（关键词匹配或轻量 LLM 调用），决定激活哪个角色和对应工具组
- [ ] 记忆软标记：在 ChromaDB metadata 中记录 source_role（产生记忆时的角色），检索时不硬过滤，仅用于排序加权；语义匹配天然区分场景，一条记忆跨多场景不受限
- [ ] 工具分组与按需加载：tools 表增加 group 字段，核心工具始终可用，领域工具按场景动态注入 function calling schema，避免 prompt 膨胀和选择困难
