...
- [x] ~~**架构师展开 vs 组长拍板缺触发标准**~~ → 已解决。协议①（结论先行，展开是支撑不是主菜）+ ②（架构师展开仅在前四项满足后按需补充）已明确展开/收敛触发标准。

- [x] ~~**误判后过度修正（hedging 反复）**~~ → 已解决。协议④（不确定就直说，不假装确定、不绕弯、不来回试探）覆盖行为层面；实际观察已改善。

- [x] ~~**跨 AI 信息同步越界边界未定义**~~ → 已解决。exchange/README.md 已明确文件归属（各 AI 只改自己的文件）+ WorkBuddy 派发权限（可在 claude/trae 文件追加任务）+ 清理权限（可判定过期内容并删除）。
