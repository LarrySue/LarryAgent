...
- [ ] **真机端到端验收（待老大亲自收尾）**：填 `config.yaml` 的 `search.brave_api_key` 真实 Brave key → 启动 LarryAgent → 聊实时问题看来源标注 → 测降级（错 key/断网出现"未能联网核实"且不崩）。逻辑层已全收口，此步为端到端收尾。
  - 注：本条自 2026-08-20 挂起约半月，**阻塞根因是外部行政依赖而非技术债/拖延**——Brave 绑卡需信用卡，老大为此申办万事达卡（制卡 → 邮寄 → 银行激活 → 又逢忙碌）。2026-09-04 卡已就绪、key 已申请，**阻塞解除**。此类依赖与上云的域名备案同性质，均非代码可解。
- [ ] **Brave 归属标注（「关于」弹窗加 Powered by Brave）**：2026-02 起 Brave 取消免费层改「$5/月信用 ≈1,000 次」，而该 **$5 信用以公开归属 Brave 为前提**（官方口径：须在 project's website / **about pages** 标注），**不标注则信用失效、1,000 次转为实际扣费**。落点现成：`docs/ui-reference.md` §5.6「关于」弹窗（v5 已落地，含 logo/版本/名单/版权行）末行加一行即可。优先级：中（等价省 $5/月）。
- 注：稿子 §八 建议的「本地计数器软上限」**经核实不写**——Brave 后台控制台本身提供 Usage limits / Spending Limit 设置（老大 2026-09-04 后台亲见，WB 已查证属实），平台侧硬限优于代码层软限，不重复造（逃熊）。
