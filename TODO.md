...
- [ ] **新增派生工作项（A 段，属 DSH-3 输入）**：**自定协议设计**——流式转发 / 会话管理 / 鉴权 / 多端同步 / 重连补帧**全部自实现**。**这是中转方案的主要成本项**，官方 Gateway 白送的恰是这部分
  - 📌 **已在 DSH-3 段正式登记为「A 段自定协议设计」** → **本条仅作派生出处留痕，执行与维护一律走 DSH-3 那条，勿双处更新**。是否勾掉本条待老大拍（勾 = 承认已转移；不勾 = DSH-2 保留一处欠账标记）
...
> | 类 | 条目 | 位置 | 判定 |
> |---|---|---|---|
> | 🔴 **A 真欠账** | `--real-api` 等价物（真实调用断言机制） | 2.4 段 + DSH-3「前置件 1」 | **未做，DSH-3 硬门禁**；派 Claude，规格复用 `dsh-local-env.md` §6 四组对照 |
> | 🟡 **B 已转移** | A 段自定协议设计 | 通信面段（派生留痕）+ DSH-3 正式条目 | 已在 DSH-3 登记，**本条待老大拍是否勾掉** |
...
>
> 1. 🔴 **沙箱方言插件生产挂载未生效** —— 插件已进 profile `node_modules`，但 `larry`/`sdk` 的 `cordis.patch.yml` **均为 `[]`** → **DSH-3 集成时必须显式挂载，否则 Trae 的修复等于没做**（挂载语法见 `log-trae.md` 一轮派发 Step 2）
