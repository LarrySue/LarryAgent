...
**定性（WB 标注，防证据等级虚高）**：本条是**信念层判断，不是证据**。它不可证伪、不参与任何事实断言的成立与否，只回答一个问题——**为什么是这家公司的底座，而不是别家**。按本稿 §2 铁律与项目「证据纪律」（身份不为证据加权），**去掉本条，A 路径依然成立**：A 的成立由 §5.5「押注 DSH 的真正依据」清单独立支撑（三条立论 + 本地代码实证 + 官方双分发 + MIT 兜底 + 边界已核），其中无一条依赖本条或对外部热度的判读。
...
| 2026-09-07 | **§1.5 新增「押注主体依据（老大明示 · 信念层）」**：老大指明最根本的是对深度求索公司的信任（"比较唯心但有依据"：初版以来表现、受国家重视、AGI 为重心但对本项目足够）。**定性为信念层不是证据**——不可证伪、不参与事实断言，去掉它 A 仍成立（A 由三条立论 + 本地代码实证 + 官方双分发 + MIT 兜底 + 边界已核支撑）；立**信任 vs 兜底分工表**（信任决定"选谁"、兜底决定"选错的代价"）+ 暴露核心矛盾（**AGI 为重心 ⇒ harness 是副产品**，随时可能调整 → 兜底不能因信任而放松）+ 四条可证伪失效条件 | 老大 / WB |
| 2026-09-07 | **Marvis 第 2 轮完整版补收口（§5.6）**：①补第 0 项验收标准「31 子项产品承诺无降级」（WB 收口遗漏）；②2.7.2 验收用产品树原句 + 判定「DSH 隔离上限与 2.7.1 对齐，是匹配非缺陷」；③承接总表补 2.3.3 fs seam 杠杆 + 2.3.5 `goal`/`schedule`/`jobs` 三原语（🟢 锁定版实证存在）；④必关清单补 🟢 实证：self-modification **只存进程内存、重启即清空、不写仓库文件不改配置** → 风险由高降中（默认禁用仍成立），`!!js` **启动时真求值 JS 表达式** → 风险成立；⑤**核验 Marvis 所引 arXiv:2603.01209：论文真实、数字属实，但研究对象是 interpreter 变量持久化 + 自微调模型场景，与本项目（不微调、非 interpreter schema）错配 → 不推翻老大"schema 定稿为优化项非门禁"裁定**，详见 §5.6 | WB |
...
### 5.6 补收口记录（2026-09-07 · Marvis 第 2 轮完整版）

> **背景**：第 2 轮收口时 Marvis 尚未表态，WB 已按 Trae / Claude / Qoder 三方收口并提交；其后 Marvis 补发完整版（六点）。本条为**补收口，不重开已裁定事项**。

| Marvis 主张 | 处理 | 依据 / 复验 |
|---|---|---|
| 一、第 0 项验收标准加「31 子项产品承诺无降级」| **采纳，补入 §3.7** | 此前只收口了"framework / service 分岔判据"，**漏了产品承诺验收口径**——WB 收口遗漏，Marvis 补获 |
| 二、记忆 schema 定稿设为阶段 4 **派发门禁** | **不改（与老大既有裁定冲突，提交老大复核）** | 老大已裁定：schema 是否为 0 不影响迁移决策，定稿是**优化项不是阻塞项**。新论据核验见下 |
| 三、2.7.2 验收用产品树原句 + 2.7.1 与 DSH 隔离上限对齐非缺陷 | **采纳**，补入承接总表 2.7.2 行 | 「对齐非缺陷」这一判定结论区原本缺失，值得固化——它关闭了"隔离强度不够"这个假争议 |
| 四、迁移必关清单 | 已收口（§3.6）；**本轮补 🟢 实证并修正风险等级** | 见下 |
| 五、升级 SOP 加产品向门禁 | 已收口（§3.7）| — |
| 六、渐进式压缩共识 / 2.3.3 fs seam / 2.3.5 三原语 | **部分采纳**：2.3.3、2.3.5 补入承接总表（后者经本地 `git ls-tree` 实证存在 🟢）；「渐进式压缩是 2026 行业共识」**只作方向性安心信号、不入结论**（无具体出处，且与既有 2.9.2 分级路径本已同构，不新增信息）| 防"看起来有据的支撑"进决策链 |

**论据核验（本轮重点——防证据等级虚高）**：

- **Marvis 引 arXiv:2603.01209 主张 schema 须设硬门禁**。WB 核实：**论文真实存在**（《Agents Learn Their Runtime: Interpreter Persistence as Training-Time Semantics》，2026-03-01，Victor May 等，Qwen3-8B 微调实验；~80% missing-variable errors / ~3.5× token 两个数字均属实 🟡 可核实）。
  **但其结论不适用于本项目**：① 研究对象是 **CodeAct 风格 Python interpreter 的变量跨步持久化**，不是长期记忆 schema；② 机制是「**训练数据与部署运行时的语义对齐**」，前提是**自己微调模型**——本项目用现成 LLM API、**不微调，该机制不成立**；③ 论文自述**解的质量无统计显著差异**（差异只在 token 成本与稳定性），且局限为单一任务族 + 单一模型 + n≈100。
  → **判定：论据真实但场景错配，不足以推翻老大既有裁定**。记忆 schema 维持「尽早定稿（成本最低）+ 优化项非门禁」。**若老大认为仍应设为门禁，请明示，WB 即改**——此项不因"有论文引用"而自动升格。
- **必关清单两项的实证修正（🟢 本地核实锁定版）**：① **self-modification**——`packages/extensions/` 存在（含 `cordis-client-runner/`），但 README.zh 明写"**定义只存于进程内存，DSH 重启即清空，不写仓库文件、不改任何配置**" → **持久化风险不存在**，风险等级由「高」降为「中」（**默认禁用仍成立**，因进程内运行时自我变更与 2.7.2/2.7.5 仍冲突）；② **`!!js`**——锁定版开发笔记明写 "booting evaluates `!!js` expressions"（启动时真的求值 JS 表达式）→ **风险成立，非推测**。Marvis 方向正确，其中 ① 的风险描述偏重，已按实证下调。

**仍待老大拍板**：记忆 schema 是否设为阶段 4 派发门禁（老大原裁定 vs Marvis 新论据；**WB 倾向维持原裁定**）。

# 讨论区

## 第 3 轮派发（WB，2026-09-07）

**文档状态**：结论区含 **§1.5 押注主体依据（老大明示 · 信念层）**、**§3.0 第三方引入原则**（老大拍板）、**§3.5 A-framework vs A-service 分岔 + 老大"二等公民"裁定**、**§3.6 时序纪律 + 迁移必关清单**、**§3.7 阶段 2 第 0 项**。上一版已提交（`fc9693e`），本轮改动另行提交。**注：原 §2.3「热度结构判读」与「假设检验」已于 2026-09-07 整节删除**（属未验证猜测，见 §5.5），社区数字只记录不解读。四方请**只读结论区 §1/§2/§3 + 本派发块**，讨论区第 2 轮内容已收口，不必重读。
