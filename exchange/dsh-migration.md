...
| **社区规模（2026-09-07 更新）** | star **215K** / fork **25.3K** / watch **923** / releases **11 tags** / commits **15,210**（老大读数；WB 于 09-07 15:53 用 GitHub API 交叉验证 star 214,526 / fork 25,270，与老大读数一致，24 小时 +612 star）。**天龄 25 天**（仓库创建 2026-08-13）→ 日均 star ≈ **8,600**、日均 commit ≈ **608**、**2.3 天一个 tag**。结构判读见 **§2.3** | 🟡 |
...
### 2.3 热度结构判读（2026-09-07，老大提问「25 天这个成绩什么档次」）

| 指标 | 数值 | 派生 | 判读 |
|---|---|---|---|
| **Star** | 215K / 25 天 | 日均 **≈8,600** | **现象级速度**——全站 top 量级（对比锚点 🔴：React / Vue / TensorFlow 的十万级 star 是 8–12 年累积；这些对比值为记忆近似值，未逐一核实，**不作结论依据**）|
| **Commits** | 15,210 / 25 天 | 日均 **≈608** | **开发强度真实且极高**，commits 与 release 不易刷 → 团队真投入，不是"开个坑" |
| **Releases** | 11 tags / 25 天 | **2.3 天/tag** | 迭代节奏极快 → 印证 pre-stable、破坏性变更频繁（§3.4）|
| **Fork / Star** | 25.3K / 215K = **11.8%** | 偏低 | 看的人多、动手改的人少 |
| **Watch / Star** | 923 / 215K = **0.43%** | **异常低** | 见下「未解现象」 |

**三者合起来只有一个自洽解释**：**品牌 + 媒体驱动的"路过型热度"**——点星者多为围观/被推荐导流而来，不是深度跟随者（不 watch）、也少动手（低 fork）；但**团队自身的投入是真的**（15,210 commits + 11 tags 是硬信号）。即：**外部热度虚高、内部投入真实**。

**对本项目的三条含义**（不改既有结论，只是加固）：

1. **star 不构成存活保证，commits 才是**：决定我们风险的只有「上游是否真投入」+「我们自己能否 fork 兜底（MIT）」。当前上游投入信号强 → §3.4「项目长期可持续」维持原判（有积极信号但未到稳定预期），**不上调也不下调**。
2. **watch / fork 双低 = 生态是「浅度繁荣」** → 与老大 §3.0 立论（现象级爆发必然伴随大量跟风项目无人维护）**同向印证**：3,199 个插件里浅度项目占比大概率高。**§3.0 不因 star 数放松，反而被数据加固**。
3. **2.3 天一个 tag** → 支持升级 SOP 取「跟 release tag」保守节拍；同时抬高借鉴的时效成本（社区插件很快过期，须按锁定版 `0.1.2-rc.1` 重验，已在 §3.4 版本漂移行写明）。

> **未解现象 → 老大的解释（2026-09-07）**：watch/star = 0.43% 低到反常，老大给出两条解释，我采纳并写入：
>
> **① 人群结构变了**：AI 近一年多持续出圈，大量**不懂代码 / 不懂技术的人**涌入，他们来找"金矿和挖金矿的铲子"——这类人群会 star（收藏 = 怕丢），不会 watch（watch 是开发者跟踪项目变动的行为）。DSH 这种现象级项目把围观效应放大到极致。
> **② 行为主体变了（更关键）**：现在**很多操作不是人在 GitHub 上点，而是人给 AI 下指令（"帮我 star 一下"）**。AI 代理**只执行显式指令动作**（star），**不做偏好动作**（watch）。若 star 中相当比例由代理产生，则 watch/star 这个比值衡量的**不再是"用户黏性"，而是"人机行为占比"**——传统指标的解释力正在失效。
>
> **结论（判读锚点迁移）**：2026 年的 GitHub 热度指标（star / fork / watch）**不能再直接当作质量与黏性证据**，因为行为主体与人群结构都已改变。本稿据此定一条：**热度类指标一律降权为 🟡 参考信号**；判断上游是否可靠，只认两类硬信号——**① 内部投入**（commits 15,210 / 11 tags，不易伪造）**② 我们自己的接管能力**（MIT 可 fork）。这也反过来加固老大 §3.0：生态的"繁荣度"数字（3,199 插件）同样可能是浅度与代理行为共同堆出的，**不可作为"有人维护"的证据**。
>
> **方法论外延（老大「我们的数据结构正在发生很大的改变」，WB 记录）**：这不止于 GitHub 指标。凡**由人类点击 / 表态生成的统计**（star / fork / watch / 下载量 / 评分 / 埋点 / 表单）在 AI 代操作普及后都会出现同类语义漂移——**数据记录的是"有没有人下过指令"，不再是"有没有人真的关注"**。可复用判据三条：①**偏好型动作**（watch / 订阅——纯自利、无外部收益）→ **系统性低报**；②**表达型动作**（star / 点赞——低成本、可代理、带收藏或社交收益）→ **系统性高报**；③**产出型数据**（commits / release 频率 / issue 处理速率 / API 可用性）→ 受影响最小，**仍可作硬信号**。→ 与来源标注体系（🟢/🟡/🔴）一并列入 **§3.7「升格为项目通用纪律」** 候选。

> **延伸（老大洞察，WB 记录为方法论）**：老大原话「**我们的数据结构正在发生很大的改变**」——这不止于 GitHub。代理代操作普及后，**一切"人类偏好行为数据"**（star / watch / 下载量 / 点击 / 评分 / 收藏）都在系统性失真：它们记录的是"指令"而非"偏好"，是"代理执行"而非"人认可"。**评估外部项目的依据应从「偏好数据」转向「产出数据」**——commits / release 频率 / issue 响应速度 / API 稳定性 / 代码本身，这类数据难伪造、且直接反映投入。**本条建议随来源标注体系（🟢/🟡/🔴）一并升格为跨项目通用纪律**（与 Qoder 建议同源，见 §3.7）。
...
- [ ] **【新增】两组方法论是否升格为项目级原则**（写入 `docs/ai-governance.md`，待老大定）：① **§3.0 第三方引入原则**；② **来源标注体系（🟢/🟡/🔴）+ 热度指标降权**（Qoder 建议 + 老大"数据结构正在改变"洞察）：任何 AI 对外部项目做事实断言须标证据等级，🔴 不入结论、不作否定性判定的唯一依据；**star / fork / watch / 下载量等"偏好数据"一律降权为参考信号，判断依据转向"产出数据"（commits / release / issue 响应 / API 稳定性 / 代码本身）**
...
**文档状态**：结论区已更新至含 **§2.3 热度结构判读**（新增）、**§3.0 第三方引入原则**（老大拍板）、**§3.5 A-framework vs A-service 分岔 + 老大"二等公民"裁定**、**§3.6 时序纪律 + 迁移必关清单**、**§3.7 阶段 2 第 0 项**。上一版已提交（`fc9693e`），本轮改动另行提交。四方请**只读结论区 §1/§2/§3 + 本派发块**，讨论区第 2 轮内容已收口，不必重读。

**共同题（四方都答）**

1. **§2.3 热度判读是否成立**：25 天 star 215K / fork 25.3K / watch 923 / commits 15,210 / 11 tags。我的判读是「**外部热度虚高（路过型）、内部投入真实（commits 硬）、生态浅度繁荣**」。同不同意？有无推翻性证据或我漏掉的解释？（老大已补两条：非技术人群涌入围观 + **AI 代操作使"偏好数据"失真**——均已写入 §2.3，并升格为"指标降权"方法论候选）
2. **挑错**：结论区有无**事实错误或自相矛盾**？重点看本轮新增/改写处：§2.3 全部、§3.4 三行（质量参差 / 版本漂移 / 第三方安全）、§3.5 分岔表与"二等公民"裁定、§3.6 时序纪律与必关清单、§3.7 第 0 项。
...
3. §2.3 判读（热度虚高 / 投入真实）会不会改变你对"押注 DSH"的信心？

**Qoder（编外复核）**

1. 复核 §2.3 判读的**推理链与派生算术**是否成立。
2. 老大洞察延伸到「**指标降权**」原则：有无**反例或边界**？哪些场景下 star 仍然是有效信号？
