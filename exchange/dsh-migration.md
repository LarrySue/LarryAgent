...
- [ ] **【第 0 项 · 最高优先 · 优先于下列全部】plugin mount 路径 + 一等/二等公民判定**——**🔄 已派发（2026-09-08）：Trae / Claude / 编外 三方并行独立执行**，各交付「一等 / 二等 / 无法判定」三选一结论（**老大裁定其为分岔判据**）。三方视角分工：Trae = 全量主路径／Claude = 边界契约层 6 类失败模式（测试本职）／编外 = **反向举证 + 判定阈值审查**。**环境隔离必做**（各自独立 `DSH_HOME`，否则 `dsh plugin --profile sdk add` 的持久 patch 会互相覆盖）。**结论冲突时以可复现实证为准，不以多数票为准**，由 WB 交叉复验：
