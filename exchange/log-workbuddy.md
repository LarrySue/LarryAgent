...
2. **✅ DSH 迁移（A 路径已拍板，进入实施规划）** — `dsh-migration.md` §三 已重写为"A 路径决策 + 实施规划"。基于老大三条立论（项目小 / 专属能力薄 / 能力建设维度升级）+ WebFetch 直接查 DSH 主仓事实校准（`0.1.3-alpha.1` / 54 包 / AGENTS.md 零论述 cloud-multi-user-tenant / `identity/` 共享匿名）。阶段 1–6 实施规划（7–12 周估算），§三.7 待办待老大拍板（克隆代码方案 / prototype 派发 / 差异化能力优先级）。
