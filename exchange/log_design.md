...
#### 仍需决策 / 回写
- [ ] **角色切换过渡动画** → 方向已定：要有但不宜显眼（无冲击感），用纯 color transition（200ms fade），不用 transform/位移。定案见 UI-Reference §9；C2 已落地，余留细节由 Trae 实现时回写 UI-Reference。
- [ ] **Trae 实现期未文档化的 UI 细节**（如新增 `--weight-semibold/bold` 等）→ 以 `tokens.css` / 组件代码为准，待点将时回写 UI-Reference §10 已知局限所列项。
