...
老大确认 8b 正向组 `COUNT = 200`（**🟢 已经命令确认**）。计数正确 = **并发写无丢失**，这比"无报错"更关键（写丢失是静默的，报错反而显性）。四项判据至此全部合格。
...
## §8 【2026-09-09 派发】CVM Linux 验证环境（主验证环境 · 执行人：老大）

> **决策背景**：老大 2026-09-09 决定**跳过 WSL 方案，直接上 CVM**。WSL 环境（§7）已配置完成且验收合格，**保留作本地快速对照，但 2.5 起的判定结论以 CVM 为准**。
>
> **执行方式**：同 §7——**老大在云控制台点选比 AI 敲命令快**。WB 只定要求与验收，不定执行方式。
> **文档定位**：本节**取代** §7 作为主验证环境规格；§7 保留为历史执行记录与本地对照基线（含可复用的验收判据）。

### 1. 为什么换成 CVM（两条实质理由，不是喜新厌旧）

1. **与生产同构**：DSH-5 的上云目标**就是 CVM**。在 CVM 上取数 = **生产同构数据**，免去"WSL 到底够不够近似"这个论证负担（该问题在 §7 里是靠"内核 6.18 够新"来间接推断的，属弱论证）。
2. **WB 可自主验证**（**协作模式的质变**）：`wsl.exe` 被沙箱拉黑 → **WB 无法在 WSL 内执行任何命令**，只能写脚本让别人跑、再读回输出（往返成本 = 人的响应时间）。而 **CVM 是独立远程主机，与 `wsl.exe` 无关，WB 可 ssh 自主跑命令与迭代**。
   - ⚠️ **待实测**：沙箱是否允许 WB 的 ssh 出站。判断上属正常远程访问（非绕过 `wsl.exe` 拉黑），但**开好机器后由 WB 实测确认**；若被拦，退回"老大代跑"模式并告知。

### 2. 选型要求（**硬要求标 ⚠️，不满足则数据不可用**）

| 项 | 要求 | 理由 |
|---|---|---|
| 镜像 | **Ubuntu 24.04 LTS** | 与 §7 WSL 同版本，可直接对照；也是主流生产选择 |
| CPU / 内存 | **2 核 4 G** 起；若 2.5 ⑤ 要跑 PyTorch CPU 版 embedding，取 **4 核 8 G** | SQLite 并发测试需要真并发；embedding 比对是 CPU 密集 |
| ⚠️ **系统盘** | **SSD 云盘**（或增强型 SSD），**50 G**；**且盘型须与 DSH-5 生产目标一致** | 见下方「存储介质的坑」 |
| 计费 | **按量付费** | 单人低频，用完关机；**关机后仅收磁盘费** |
| 地域 | 就近；若 DSH-5 目标地域已定则**按目标地域** | 减少变量 |
| 带宽 | 按使用流量，或 1–5 Mbps 固定 | 仅传代码与结果，需求很低 |

**存储介质的坑（本项最关键，别想当然）**：

> **云盘是分布式网络存储，`fsync` 延迟与本地 NVMe 不是一个量级。**
> 而 DSH-2.5 ① 测的正是 **SQLite WAL + 文件锁的并发行为，直接受 fsync 延迟影响**——延迟会改变锁竞争的时间窗口。
>
> **但这对本项目反而是好事**：若生产（DSH-5）同样用云盘，则 **CVM 上测到的数据就是生产数据**，比 WSL 的本地 NVMe**更准**。
> → 所以正确做法不是"挑最快的盘"，而是**挑与生产一致的盘**，并把盘型记录进结论。

### 3. 网络与访问

- **安全组**：放行 **22**（SSH）；若要 Windows 侧访问 CVM 上的 HTTP 服务（DSH-3 验 Gateway），放行 **8000–9000** 段。
- **登录方式**：用 **SSH 密钥**（创建时绑定或后续导入）。
- **开好后给 WB 四样**：公网 IP、用户名（Ubuntu 镜像一般 `ubuntu`）、SSH 端口、密钥在本机的路径。**给完 WB 自己连，不用你代跑。**

### 4. 验收（判据复用 §7，不另起炉灶）

**同一套正反两组，在两个环境跑同样命令，结论可直接对照**：

| 组 | 条件 | 期望 |
|---|---|---|
| 环境 | `df -T .` | ext4 |
| 8a 反向 | `busy_timeout=0` + 并发 100 写 | **出现** `database is locked`（证明锁在拦） |
| 8b 正向 | `busy_timeout=10s` + 并发 100 写 | **无**报错，且 `COUNT` = 200（无写丢失） |

**额外记录**（本次 CVM 特有）：盘型、`uname -r`、SQLite CLI 与 Node 内嵌两处版本、`nproc` / `free -g`。

### 5. 已知坑

1. **成本**：按量不贵，但**忘记关机会持续计费** → 不用时**关机**（不必销毁，数据要留着做对照）。
2. **这是公网机器**：不放生产凭据与真实数据。
3. **时钟**：云服务器一般 NTP 正常，优于 WSL2（WSL2 休眠会漂移）；仍建议验前 `date` 对表。
4. **两份环境并存时注意结论归属**：同一份数据要标注**取自 CVM 还是 WSL**，别混用。
