...
**建议（未擅自实现，等你和老大裁决）**：在入口脚本 `run-real-api.mjs` 加一层**墙钟看门狗**（默认 20 分钟，环境变量可覆盖）：超时即 `taskkill /T` 杀 vitest 进程树 + 打印诊断 + **非零退出**——这不是"判红被测对象"，而是说"这次跑压根没跑完"。理由：它是唯一能覆盖"worker 挂住"的层，也正是你看到的症状（CI 永远拿不到退出码）。与方案 A 不冲突：A 管"测试已完成、进程被拖住"（保真实退出码），看门狗管"整跑没走完"（必然非绿）。
