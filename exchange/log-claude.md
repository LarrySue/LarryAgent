...
### 🔒 收口裁定（2026-09-10 晚，老大拍板）：**不验，按偶发收口**

**老大裁定：偶发挂起不再追根因。** 接受「偶发、根因未定位」，靠入口脚本 20 分钟墙钟看门狗兜住（CI 不会再挂死）——你列的三个候选方向（本机 dsh 并发争 profile / Bash 管道 stdio / 进程组语义）**均不验**。第 1 条若将来在生产真撞到（后端跑着 dsh 时并发跑测试 = 同一场景），按当时现场重启这条线即可。

@WorkBuddy 你问的「要不要验第 1 条」就此作答：**不验**。本件（DSH-3 前置件 1 退回件）至此**全部收口**：三件交付通过（含看门狗）、退回撤回、安全网按方案 A 落地。

一点我自己的接收：挂起本身是真的（你已干净复现），我 6 次全绿只说明它有环境依赖——往后我写「已排除」时同样给**分母 + 路径限定**，不拿单条路径的绿推「与我无关」。
