...
- **UI 精简第 4 轮** ✅ 已提交（Trae，2026-08-24，老大直接对接）：
  - 删 TopBar `◀` 侧栏折叠按钮（桌面端点击无效，折叠功能未实现，条目移出 TODO）；移动端抽屉逻辑保留
  - 删会话列表角色色点（原为全局角色色的重复展示；会话级角色归属需数据模型支撑，新增 TODO「角色归属设计」暂缓）
  - UI-Reference §5.4 回写对齐（SidebarItem = 标题 + 时间戳）
  - 数据维护：清理 33 条最早会话（08-15/08-19 的 echo/ghost/loop/limit 等测试数据，经 DELETE API 级联删除），保留全部真实会话
- **UI 调整第 5 轮：多色品牌文字** ✅ 已提交（Trae，2026-08-24，老大直接对接）：
  - 输入框占位符精简：去掉"(Enter 发送, Shift+Enter 换行)"快捷键提示，仅留"输入消息..."
  - 调试台升级：新增逐字母文字调色（LarryAgent 多色混搭方案在此定稿）+ 画布 48×48→48×60（文字下置圆环下方）+ 默认预览尺寸 384px
  - 多色品牌文字落地：新组件 `BrandText.vue`（L `#f0f0f0` / a-r-r-y `#707070` / A `#5cadff` 锚点蓝 / g-e-n `#707070` / t `#c83c3c` 朱红），应用于侧栏标题 + 欢迎页大标题 + 移动端标题
  - 顶部栏无会话时显示"新会话"（不再显示 LarryAgent 字样）
  - 尺寸调整：欢迎页 logo/标题放大 2 倍（96px / 40px）；侧栏 logo 24→32px（+30%）
- **会话重命名（三点菜单）** ✅ 已提交（Trae，2026-08-24，老大直接对接）：
